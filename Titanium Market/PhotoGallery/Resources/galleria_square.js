...
function squareThumb(imagePath) {
	var imageView = Ti.UI.createImageView({
		image: imagePath
	});
	var blob = imageView.toImage();
	//attributes imageAsThumbnail(size,bordeSize,cornerRadius);
	return blob.imageAsThumbnail(75,1,5);
}
...
for (var i in images) {
	...
	img = Titanium.UI.createImageView({
		image: squareThumb(images[i].image),
		width: 75,
		height: 75,
		top: top,
		left: left,
