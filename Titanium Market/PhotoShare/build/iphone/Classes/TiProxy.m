...
//Rare exceptions to indicate a bug in the _photoshare code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_photoshare = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_photoshare]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.photoshare.module"
