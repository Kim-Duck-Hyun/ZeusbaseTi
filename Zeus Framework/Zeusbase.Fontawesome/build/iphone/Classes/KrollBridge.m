...
NSString * ZeusbaseFontawesome$ModuleRequireFormat = @"(function(exports){"
		"var __OXP=exports;var module={'exports':exports};%@;\n"
		"if(module.exports !== __OXP){return module.exports;}"
		"return exports;})({})";


@implementation ZeusbaseFontawesomeObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_zeusbasefontawesome);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create ZeusbaseFontawesome global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "ZeusbaseFontawesome" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_zeusbasefontawesome = [[ZeusbaseFontawesomeObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_zeusbasefontawesome];
	
	NSString *_zeusbasefontawesomeNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _zeusbasefontawesomeNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_zeusbasefontawesome valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:ZeusbaseFontawesome$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.zeusbasefontawesome.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.zeusbasefontawesome.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
